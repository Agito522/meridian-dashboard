...
- **Earnings ATM straddle calculator** — standalone Traditional Chinese page (`earnings-move.html`) for C泰龍’s 1-minute implied-move method: `(C+P)/S` range plus a 2% call/put hedge-value bias. Prefills the NVDA worked example. Linked discreetly from the dashboard footer. Method workings: [`docs/earnings-atm-move.md`](docs/earnings-atm-move.md).
...
├── docs/earnings-atm-move.md  # Method workings (formulas + NVDA example)
